  ...
  return (
    <section id="products" ref={containerRef} className="py-24 overflow-hidden relative z-10 bg-black/60 backdrop-blur-md">
      <div className="max-w-7xl mx-auto pl-6 pr-6 md:pl-40 md:pr-12 mb-16 flex justify-between items-end">
         <div>
            <span className="text-[#fbcfe8] text-xs font-bold tracking-[0.3em] uppercase block mb-4">The Collection</span>
            <h2 className="text-4xl md:text-6xl font-serif text-white">Signature Flavors.</h2>
         </div>
        ...
        {products.map((p, i) => (
          ...
          >
             ...
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-64 bg-black/40 border border-white/5 backdrop-blur-sm flex items-center justify-center">
                 <span className="text-6xl font-serif text-white/10 rotate-90 whitespace-nowrap tracking-widest pointer-events-none select-none">GLACÉ {p.id}</span>
             </div>
             ...
             <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black via-black/80 to-transparent">
                <div className="flex justify-between items-end">
                   ...
                   <span className="text-lg font-mono text-[#fbcfe8]">{p.price}</span>
                </div>
             </div>
          </motion.div>
        ))}
