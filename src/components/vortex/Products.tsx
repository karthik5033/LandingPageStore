      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-20">
          <div>
            ...
            <h2 className="text-5xl md:text-7xl font-normal text-white premium-text-shadow leading-[1.1]" style={{ fontFamily: "'Krona One', sans-serif" }}>Three origins. <span className="italic text-[#ccff00]">One velocity.</span></h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {collection.map((p, i) => (
            <motion.div key={p.name} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.9 }}
              ...
              <h3 className="text-3xl md:text-4xl font-normal text-white premium-text-shadow mb-2 tracking-wide" style={{ fontFamily: "'Krona One', sans-serif" }}>{p.name}</h3>
