    ...
    <section id="craft" className="relative z-10 py-40 px-8 md:px-16">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="mb-28">
          <div className="flex items-center gap-6 mb-8"><div className="w-16 h-[1px] bg-white" /><span className="text-xs font-medium tracking-[0.5em] text-white/60 uppercase">The open pipeline</span></div>
          <h2 className="text-5xl md:text-7xl font-light text-white premium-text-shadow leading-[1.1] max-w-3xl" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>From receipt <span className="italic text-white/40">to wrapper.</span></h2>
        </motion.div>
        <div className="space-y-1">
          {steps.map((s, i) => (
            <motion.div key={s.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.9 }}
              className="group border border-white/[0.06] bg-transparent backdrop-blur-sm p-10 md:p-14 hover:border-white/10 transition-all duration-700">
              <div className="flex flex-col md:flex-row md:items-start gap-8">
                <span className="text-5xl font-light text-white/10 group-hover:text-white/20 transition-colors duration-700 shrink-0" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{s.num}</span>
                <div className="flex-1"><h3 className="text-2xl md:text-3xl font-light text-white premium-text-shadow mb-4 group-hover:text-white/70 transition-colors duration-500" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{s.title}</h3><p className="text-white/50 text-lg font-light leading-[1.9]">{s.body}</p></div>
              </div>
            </motion.div>
          ))}
        </div>
