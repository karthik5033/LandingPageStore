    ...
    <section id="products" className="py-32 overflow-hidden relative z-10 bg-transparent border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 md:pl-40 md:pr-12 mb-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
         ...
         >
            ...
            <h2 className="text-5xl md:text-7xl font-serif text-white tracking-tight">Signature <span className="italic text-gray-400 font-light">Swirls.</span></h2>
         </motion.div>
         <motion.button 
           whileHover={{ x: 10 }}
      ...
      <div className="flex gap-8 px-6 md:pl-40 pb-20 overflow-x-auto no-scrollbar snap-x snap-mandatory">
        {products.map((p, i) => (
          ...
          >
             <div className="relative h-[550px] bg-black/40 backdrop-blur-xl border border-white/20 rounded-lg overflow-hidden transition-all duration-500 hover:border-[#d8b4e2]/50">
                {/* Main Content Container */}
                <div className="absolute inset-0 p-10 flex flex-col justify-between z-10">
                   <div className="flex justify-between items-center">
                      <div className="w-8 h-[1px] bg-white/40 group-hover:w-16 group-hover:bg-[#d8b4e2] transition-all duration-500" />
                      <span className="text-[10px] font-mono text-gray-400 tracking-widest uppercase">Variant 0{p.id}</span>
                   </div>

                   <div className="space-y-6">
                      <div className="space-y-3">
                        <h3 className="text-4xl font-serif text-white tracking-tight group-hover:text-[#d8b4e2] transition-colors duration-500">
                          {p.name}
                        </h3>
                        <p className="text-xs text-[#d8b4e2] uppercase tracking-[0.2em] font-bold">
                          {p.notes}
                        </p>
                      </div>
                      ...
                      <div className="pt-6 flex justify-between items-center border-t border-white/10 mt-6">
                         ...
                         <motion.div 
                           ...
                           className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-black hover:bg-[#d8b4e2] transition-colors"
                         >
