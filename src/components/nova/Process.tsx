    ...
    <section id="craft" className="relative z-10 py-40 px-8 md:px-16">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="mb-28">
          ...
          <h2 className="text-5xl md:text-7xl font-light text-white premium-text-shadow leading-[1.1] max-w-3xl" style={{ fontFamily: "'Fraunces', serif" }}>From highland <br /><span className="italic text-[#e0e7ff]">to horizon.</span></h2>
        </motion.div>
          ...
          {steps.map((s, i) => (
            <motion.div key={s.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.9 }}
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-8">
                ...
                <div className="flex-1"><h3 className="text-2xl md:text-3xl font-light text-white premium-text-shadow mb-4 group-hover:text-[#e0e7ff] transition-colors duration-500" style={{ fontFamily: "'Fraunces', serif" }}>{s.title}</h3><p className="text-[#8a8fa8] text-lg font-light leading-[1.9]">{s.body}</p></div>
              </div>
            </motion.div>
          ))}
        </div>
