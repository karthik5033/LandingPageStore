    ...
    <section id="atelier" className="relative z-10 py-40 px-6 md:px-16">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-10%' }} transition={{ duration: 1 }} className="mb-28">
          ...
          <h2 className="text-5xl md:text-7xl font-light text-white premium-text-shadow leading-[1.1] max-w-3xl" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Light is <br /><span className="italic text-[#ffd700]">our ingredient.</span></h2>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-20">
          {pillars.map((p, i) => { const Icon = p.icon; return (
            <motion.article key={p.label} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-5%' }} transition={{ delay: i * 0.08, duration: 0.9 }} className="group">
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-2xl font-normal text-white premium-text-shadow mb-1 tracking-wide group-hover:text-[#ffd700] transition-colors duration-500" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{p.title}</h3>
