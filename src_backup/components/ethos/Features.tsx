    ...
    <section id="atelier" className="relative z-10 py-40 px-6 md:px-16">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="mb-28">
          <div className="flex items-center gap-6 mb-8"><div className="w-16 h-[1px] bg-[#1a1a1a]" /><span className="text-xs font-medium tracking-[0.5em] text-[#1a1a1a]/60 uppercase">The six principles</span></div>
          <h2 className="text-5xl md:text-7xl font-light text-[#1a1a1a] leading-[1.1] max-w-3xl" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Transparency is <span className="italic text-[#1a1a1a]/40">our product.</span></h2>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-20">
          {pillars.map((p, i) => { const Icon = p.icon; return (
            <motion.article key={p.label} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.9 }} className="group bg-white/60 backdrop-blur-md border border-black/10 p-8 rounded-xl">
              <div className="flex items-start gap-6"><div className="flex flex-col items-center gap-3 pt-1"><span className="text-xs tracking-[0.3em] text-[#1a1a1a]/30 font-mono">{p.label}</span><div className="w-[1px] h-8 bg-[#1a1a1a]/10 group-hover:h-14 group-hover:bg-[#1a1a1a]/30 transition-all duration-700" /><Icon className="w-4 h-4 text-[#1a1a1a]/20 group-hover:text-[#1a1a1a]/60 transition-colors duration-500" /></div>
                <div className="flex-1"><h3 className="text-2xl font-light text-[#1a1a1a] mb-1 tracking-wide group-hover:text-[#1a1a1a]/70 transition-colors duration-500" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{p.title}</h3><span className="text-xs tracking-[0.3em] text-[#1a1a1a]/20 uppercase block mb-5">{p.subtitle}</span><p className="text-[#1a1a1a]/50 text-lg font-light leading-[1.9]">{p.body}</p></div></div>
            </motion.article>
          ); })}
        </div>
      </div>
