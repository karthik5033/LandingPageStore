      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-20">
          <div>
            ...
            <h2 className="text-5xl md:text-7xl font-light text-white leading-[1.1]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Three altitudes. <br /><span className="italic text-[#ffd700]">One radiance.</span></h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {collection.map((p, i) => (
            <motion.div key={p.name} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.9 }}
              ...
              <h3 className="text-3xl md:text-4xl font-light text-white mb-2 tracking-wide" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{p.name}</h3>
              ...
              <div className="flex items-center justify-between mt-auto pt-8 border-t border-white/[0.06]">
                <div><span className="text-2xl font-light text-white" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{p.price}</span><span className="text-xs text-white/30 ml-2 tracking-wider">{p.weight}</span></div>
